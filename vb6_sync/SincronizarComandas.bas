...
Public Sub SincronizarComandas(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    rs.Open "SELECT * FROM comanda WHERE Enviada_MySql = 0 AND year(Fecha) >= 2025 LIMIT " & Var_Limit_Registros, conn
    ...
    respuesta = ApiPost("/sync/push/orders", json)
    ...
    Var_Caption_Error = "Comandas Env.: " & sc.Eval("r.total_saved") & _
