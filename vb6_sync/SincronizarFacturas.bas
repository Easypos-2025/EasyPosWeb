...
' Columnas locales:
'   Nro_Factura, Fecha, Valor_Efectivo, Descuento, Cedula,
'   Empleado, Anulada, Pago_Iva, Arreglo, Valor_T_Credito,
'   Valor_T_Debito, Propina, Turno, Hora, Hora_Texto,
'   Propina_Extra, Valor_Sin_Propina, Analizada, Tipo_Moneda,
'   Valor_Extrangero, Factura_Manual, Id_Resolucion,
'   Factura_Reserva, Factura_Domicilio, Enviada_MySql
...
Public Sub SincronizarFacturas(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    Do While Not rs.EOF
        ...
        json = json & """adjustment"":"         & Nz(rs("Arreglo"), 0)                                                    & ","
        ...
        json = json & """foreign_amount"":"     & Replace(CStr(Nz(rs("Valor_Extrangero"), 0)), ",", ".")                  & ","
        json = json & """manual_invoice"":"     & CInt(Nz(rs("Factura_Manual"), 0))                                       & ","
        json = json & """resolution_id"":"      & Nz(rs("Id_Resolucion"), 0)                                              & ","
        json = json & """reservation_invoice"":"  & """" & EscapeJson(CStr(Nz(rs("Factura_Reserva"), "0")))               & ""","
        json = json & """delivery_invoice"":"   & CInt(Nz(rs("Factura_Domicilio"), 0))
        json = json & "}"
        sep = ","
