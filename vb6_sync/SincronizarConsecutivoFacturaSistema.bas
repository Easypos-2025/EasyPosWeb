' ============================================================
' SincronizarConsecutivoFacturaSistema
' Endpoint: POST /api/pos/sync/push/consecutivo-factura-sistema
' Tabla local VB6: consecutivo_factura_sistema
' Tabla servidor:  consecutivo_factura_sistema
...
' ============================================================
Public Sub SincronizarConsecutivoFacturaSistema(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    On Error GoTo ErrHandler

    Dim conn As Object
    Set conn = GetConn(Var_Sql_Base_Datos_Principal_Sede)
    Var_Tabla_Error = "SincronizarConsecutivoFacturaSistema"

    ' -- 1. Leer pendientes (lotes) -------------------------
    Dim rs As Object
    Set rs = CreateObject("ADODB.Recordset")
    rs.Open "SELECT * FROM consecutivo_factura_sistema WHERE Enviada_MySql = 0 AND year(Fecha) >= 2025 LIMIT " & Var_Limit_Registros, conn
    ...
    respuesta = ApiPost("/sync/push/consecutivo-factura-sistema", json)
    ...
    If idList <> "" Then
        conn.Execute "UPDATE consecutivo_factura_sistema SET Enviada_MySql = 1 " & _
    ...
    Var_Caption_Error = "ConsecSistema Env.: " & sc.Eval("r.total_saved") & _
