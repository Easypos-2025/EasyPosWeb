...
' ============================================================
Public Sub SincronizarGastos(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    rs.Open "SELECT * FROM gastos WHERE Enviada_MySql = 0 AND year(Fecha_Gasto) >= 2025 LIMIT " & Var_Limit_Registros, conn
    ...
    respuesta = ApiPost("/sync/push/expenses", json)
    ...
    If idList <> "" Then
        conn.Execute "UPDATE gastos SET Enviada_MySql = 1 " & _
    ...
    Var_Caption_Error = "Gastos Env.: " & sc.Eval("r.total_saved") & _
