...
Public Sub SincronizarPlatos(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    Do While Not rs.EOF
        ...
        json = json & """procedure"":"                 & """" & EscapeJson(Nz(rs("Procedimiento"), ""))        & ""","
        json = json & """description"":"               & """" & EscapeJson(Nz(rs("Descripcion_Plato"), ""))    & ""","
        ...
        json = json & """pre_preparation"":"           & Nz(rs("Preparacion_Previa"), 0)                        & ","
        json = json & """offer"":"                     & Nz(rs("Ofrecer"), 0)                                   & ","
        json = json & """offer_priority"":"            & Nz(rs("Prioridad_Ofrecer"), 0)                         & ","
        ...
        json = json & """ask_sale_price"":"            & Nz(rs("Pedir_Valor_Venta_Producto"), 0)                & ","
        json = json & """ask_product_description"":"   & Nz(rs("Pedir_Descripcion_Producto"), 0)
        json = json & "}"
        sep = ","
        rs.MoveNext
    Loop
