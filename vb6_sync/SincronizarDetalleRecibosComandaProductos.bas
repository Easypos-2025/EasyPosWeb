...
' ============================================================
Public Sub SincronizarDetalleRecibosComandaProductos(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    rs.Open "SELECT * FROM recibos_detalle_comanda_producto WHERE Enviada_MySql = 0 AND year(Fecha) >= 2025 LIMIT " & Var_Limit_Registros, conn
    ...
    respuesta = ApiPost("/sync/push/receipt-order-detail-products", json)
    ...
    If comandas <> "" Then
        conn.Execute "UPDATE recibos_detalle_comanda_producto SET Enviada_MySql = 1 " & _
    ...
    Var_Caption_Error = "Det.Rec.Prod Env.: " & sc.Eval("r.total_saved") & _
