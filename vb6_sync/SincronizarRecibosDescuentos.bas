...
' ============================================================
Public Sub SincronizarRecibosDescuentos(Var_Id_Company_Envio As Integer, Var_Limit_Registros As Variant)
    ...
    rs.Open "SELECT * FROM recibos_descuentos WHERE Enviada_MySql = 0 LIMIT " & Var_Limit_Registros, conn
    ...
    Do While Not rs.EOF
        ...
        json = json & """receipt_number"":"    & """" & Nz(rs("Factura"), "")                       & ""","
    ...
    respuesta = ApiPost("/sync/push/receipt-discounts", json)
    ...
    If idList <> "" Then
        conn.Execute "UPDATE recibos_descuentos SET Enviada_MySql = 1 " & _
    ...
    Var_Caption_Error = "Desc.Recibo Env.: " & sc.Eval("r.total_saved") & _
