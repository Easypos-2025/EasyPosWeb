  ...
  <div class="ix-wrap">
    ...
    <div class="filters-bar">
      ...
      <ExportToolbar
        :data="exportData"
        :columns="exportCols"
        filename="salidas-inventario"
        title="Salidas de Inventario"
      />
      <button class="btn-nueva" @click="openCreate">
        <i class="bi bi-plus-lg"></i><span class="btn-lbl"> Nueva Salida</span>
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div v-for="group in groupedByCategory" :key="group.name" class="cat-group">
        ...
        <div v-show="openCat === group.name" class="cards-list">
          <div v-for="r in group.items" :key="r.id" class="sc">
            <div class="sc-status">
              <span class="sb b-exit"><i class="bi bi-arrow-down-circle-fill"></i> Salida</span>
            </div>
            <div class="sc-main">
              <div class="sc-name">
                {{ r.item_name }}
            ...
            <div class="sc-qty">
              <span class="fw-b c-red">-{{ fmt(r.cantidad) }}</span>
              <span class="sc-unit">{{ r.unit_name }}</span>
            </div>
            <div class="sc-act">
              <button class="btn-del-row" @click.stop="remove(r)" title="Anular salida">
      ...
      <div v-if="!filteredRows.length" class="empty-c">Sin registros de salidas</div>
    </template>

    <!-- Modal nueva salida -->
    <div v-if="showModal" class="modal-bg" @click.self="showModal=false">
      <div class="iv-modal">
        <h6 class="modal-title"><i class="bi bi-arrow-down-circle me-1 c-red"></i> Nueva Salida de Inventario</h6>
        ...
        <div class="form-group">
          ...
          <input type="text" v-model="form.observacion" class="form-control" placeholder="Merma, devolución, muestra…" />
        </div>
...
async function load() {
  loading.value = true
  try {
    rows.value = (await api.get('/api/inventory/exits')).data
    await nextTick()
...
async function save() {
  saving.value = true
  try {
    await api.post('/api/inventory/exits', form.value)
...
.btn-nueva:hover { background: #b91c1c; }
...
.cat-hdr:hover { background: #b91c1c; }
...
/* Cards */
.cards-list { display: flex; flex-direction: column; border: 1px solid #fecdd3; border-top: none; border-radius: 0 0 8px 8px; overflow: hidden; }
.sc {
...
.b-exit { background: #fee2e2; color: #dc2626; }
...
.btn-confirm { background: #dc2626; color: #fff; border: none; border-radius: 8px; padding: 10px 20px; cursor: pointer; font-size: .87rem; font-weight: 700; display: flex; align-items: center; gap: 7px; }
