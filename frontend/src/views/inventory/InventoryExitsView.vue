<template>
  <div class="inv-view">
    <div class="iv-header">
      <h5 class="iv-title">Salidas de Inventario</h5>
      <button class="btn-primary-sm" @click="openCreate">+ Nueva Salida</button>
    </div>

    <div class="iv-summary">
      <div class="summary-card red">
        <span class="sc-label">Total salidas (período)</span>
      ...
      <table class="iv-table">
        ...
        <tbody>
          ...
          <tr v-for="r in rows" :key="r.id">
            ...
            <td class="text-end fw-bold text-red">-{{ fmt(r.cantidad) }}</td>
            <td>{{ r.unit_name }}</td>
            <td class="small text-muted">{{ r.cod_empleado }}</td>
            <td class="small text-muted">{{ r.observacion }}</td>
            <td>
              <button class="btn-del" @click="remove(r)" title="Anular salida">✕</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showModal" class="iv-modal-bg" @click.self="showModal=false">
      <div class="iv-modal">
        <h6 class="modal-title">Nueva Salida de Inventario</h6>
        ...
        <div class="form-group">
          ...
          <input type="text" v-model="form.observacion" class="form-control" placeholder="Merma, devolución a proveedor, muestra…" />
        </div>
...
async function load() {
  loading.value = true
  try { rows.value = (await api.get('/api/inventory/exits')).data }
