        ...
        <div class="modal-body">
          ...
          <template v-if="modal.connection_type === 'usb'">
            <div class="detector-box">
              <p class="detector-hint">
                <i class="bi bi-info-circle me-1"></i>
                Conecta la impresora por USB, luego haz clic en detectar.
              ...
              <div v-if="modal.usb_device_id" class="device-found">
                ...
                Dispositivo: <strong>{{ modal.usb_device_id }}</strong>
              </div>
              <div v-if="errorUSB" class="device-error">
                <i class="bi bi-exclamation-triangle me-1"></i>{{ errorUSB }}
              </div>
            </div>
          </template>
          ...
          <template v-if="modal.connection_type === 'network'">
            <div class="detector-box">
              <p class="detector-hint">
                <i class="bi bi-info-circle me-1"></i>
                Busca impresoras visibles en tu red (WiFi o cable). O ingresa la IP directamente.
              </p>
              ...
              <div class="scan-row">
                ...
                <button class="btn-detect" :disabled="escaneando" @click="escanearRed">
                  <span v-if="escaneando">
                    <span class="spinner-border spinner-border-sm me-1"></span>
                    Escaneando {{ scanProgress }}/{{ scanTotal }}…
                  </span>
                  <span v-else><i class="bi bi-radar me-1"></i>Escanear red</span>
                </button>
              </div>

              <!-- Resultados del escaneo -->
              <div v-if="encontradas.length" class="found-list">
                <p class="found-title">{{ encontradas.length }} impresora(s) encontrada(s) — haz clic para seleccionar:</p>
                <button
                  v-for="ip in encontradas" :key="ip"
              ...
              <div v-else-if="escaneoHecho && !escaneando" class="device-error">
                No se encontraron impresoras en esa subred. Intenta con otra o ingresa la IP manualmente.
              </div>

              <!-- IP manual -->
              <div class="ip-manual-wrap">
                <div class="campo">
                  <label>IP manual (opcional)</label>
                  ...
                  <span v-if="estadoIP === 'online'" class="ip-status ip-online">
                    <i class="bi bi-circle-fill me-1"></i>En línea
                  </span>
                  <span v-else-if="estadoIP === 'offline'" class="ip-status ip-offline">
                    <i class="bi bi-circle-fill me-1"></i>No responde
                  </span>
                </div>
              </div>
            </div>
          ...
          <template v-if="modal.connection_type === 'bluetooth'">
            <div class="detector-box">
              <p class="detector-hint">
                <i class="bi bi-info-circle me-1"></i>
                Enciende la impresora Bluetooth y asegúrate que esté en modo pairing.
                Requiere Chrome o Edge con HTTPS.
              </p>
              <button class="btn-detect btn-detect--bt" :disabled="detectandoBT" @click="detectarBluetooth">
                <span v-if="detectandoBT">
                  <span class="spinner-border spinner-border-sm me-1"></span>Buscando dispositivos…
                </span>
                <span v-else>
                  <i class="bi bi-bluetooth me-1"></i>Buscar impresora Bluetooth
                </span>
              </button>
              <div v-if="modal.bluetooth_address" class="device-found">
                <i class="bi bi-check-circle-fill text-success me-1"></i>
                Dispositivo vinculado: <strong>{{ modal.name }}</strong>
                <small class="d-block text-muted">ID: {{ modal.bluetooth_address }}</small>
              </div>
              <div v-if="errorBT" class="device-error">
                <i class="bi bi-exclamation-triangle me-1"></i>{{ errorBT }}
              </div>
            </div>
          </template>
...
const placeholderNombre = computed(() => ({
  usb:       'Ej: Tiquetera USB Cocina',
  network:   'Ej: Tiquetera Barra WiFi',
  bluetooth: 'Ej: Tiquetera BT Mesa',
...
const modal = ref({
  ...
  name: '', connection_type: 'network',
  ip: '', bluetooth_address: '', usb_device_id: '',
  is_active: 1,
})
...
const estadoIP    = ref('')   // '' | 'online' | 'offline'
...
function abrirModal(imp=null) {
  ...
  modal.value = imp
    ? {
        visible: true, id: imp.id,
        name: imp.name,
        connection_type: imp.connection_type || 'network',
        ip: imp.ip || '',
        bluetooth_address: imp.bluetooth_address || '',
        usb_device_id: imp.usb_device_id || '',
        is_active: imp.is_active,
      }
    : { visible:true, id:null, name:'', connection_type:'network', ip:'', bluetooth_address:'', usb_device_id:'', is_active:1 }

  // Auto-detect subred desde IP guardada
  if (imp?.ip) {
    const parts = imp.ip.split('.')
    if (parts.length === 4) subred.value = parts.slice(0, 3).join('.')
  }
}
function cerrarModal() { modal.value.visible=false; resetDetectores() }
function cambiarTipo(t) {
  modal.value.connection_type = t
  resetDetectores()
}
...
async function guardar() {
  ...
  try {
    const p = {
      ...
      connection_type:    modal.value.connection_type,
      ip:                 modal.value.ip || null,
      bluetooth_address:  modal.value.bluetooth_address || null,
      usb_device_id:      modal.value.usb_device_id || null,
...
async function detectarUSB() {
  errorUSB.value = ''
  if (!navigator.usb) {
    errorUSB.value = 'Tu navegador no soporta Web USB. Usa Chrome o Edge.'
    return
  }
  detectandoUSB.value = true
  try {
    // classCode 7 = Printer class USB
    ...
    if (e.name !== 'NotFoundError') errorUSB.value = e.message || 'No se pudo conectar al dispositivo'
  } finally {
    detectandoUSB.value = false
  }
}

// ── RED: detección de subred + escaneo ────────────────────────────────────────
...
async function escanearRed() {
  encontradas.value = []
  escaneoHecho.value = false
  estadoIP.value = ''

  // Auto-detectar subred si no está escrita
  if (!subred.value) {
    const detected = await detectarSubredLocal()
    if (detected) subred.value = detected
  }

  if (!subred.value || !/^\d{1,3}\.\d{1,3}\.\d{1,3}$/.test(subred.value)) {
    alert('Ingresa una subred válida (ej: 192.168.1)')
    return
  }
  ...
  const found = []
  const BATCH = 40  // conexiones paralelas por lote

  for (let start = 1; start <= 254; start += BATCH) {
    const end = Math.min(start + BATCH - 1, 254)
    const batch = []
    for (let i = start; i <= end; i++) {
      batch.push(probarConexion(`${subred.value}.${i}`))
    }
    const results = await Promise.all(batch)
    results.forEach((ok, idx) => {
      if (ok) found.push(`${subred.value}.${start + idx}`)
    })
    scanProgress.value = Math.min(end, 254)
  }

  encontradas.value = found
  escaneoHecho.value = true
  escaneando.value = false
}

async function probarConexion(ip, timeout = 600) {
  // Intenta WebSocket a puerto 9100 (protocolo de impresoras térmicas)
  return new Promise(resolve => {
    try {
...
async function probarIP() {
  ...
  estadoIP.value = ok ? 'online' : 'offline'
...
async function detectarBluetooth() {
  ...
  if (!navigator.bluetooth) {
    errorBT.value = 'Tu navegador no soporta Bluetooth. Usa Chrome o Edge con HTTPS.'
    return
  }
  ...
  try {
    const device = await navigator.bluetooth.requestDevice({
      ...
      optionalServices: [
        '000018f0-0000-1000-8000-00805f9b34fb',  // Servicio común impresoras BT
        '49535343-fe7d-4ae5-8fa9-9fafd205e455',  // Otro servicio común
        'e7810a71-73ae-499d-8c15-faa9aef0c3f2',  // ZJ-58 y similares
      ],
    })
    modal.value.bluetooth_address = device.id
    modal.value.name = device.name || modal.value.name || 'Impresora Bluetooth'
  } catch(e) {
    if (e.name !== 'NotFoundError') errorBT.value = e.message || 'No se pudo conectar al dispositivo'
  } finally {
    detectandoBT.value = false
  }
}
</script>
...
.btn-detect    { display:flex;align-items:center;justify-content:center;gap:6px;background:#1d4ed8;color:#fff;border:none;border-radius:8px;padding:10px 16px;font-size:13px;font-weight:600;cursor:pointer;transition:opacity .15s; }
