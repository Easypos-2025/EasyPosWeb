<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title"><i class="bi bi-receipt me-2"></i>Conceptos de Gasto</h1>
        <p class="page-subtitle">Catálogo de conceptos para registro de gastos en tareas</p>
      </div>
    ...
    <div v-if="filtered.length === 0" class="empty-state">
      ...
      <p>{{ search ? 'Sin resultados para "' + search + '"' : 'No hay conceptos de gasto registrados' }}</p>
    ...
    <div v-if="modal" class="modal-overlay" @click.self="modal = false">
      <div class="modal-card">
        <div class="modal-header">
          <h3>{{ editing ? 'Editar concepto' : 'Nuevo concepto de gasto' }}</h3>
          <button class="modal-close" @click="modal = false"><i class="bi bi-x-lg"></i></button>
        </div>
        <div class="modal-body">
          <div class="fg">
            <label>Nombre *</label>
            <input v-model="form.name" class="form-control" placeholder="Ej: Transporte, Alimentación, Herramientas..." autofocus />
          </div>
          <div class="fg mt-3">
...
async function load() {
  try {
    const res = await api.get("/conceptos-gastos/")
...
async function save() {
  ...
  try {
    if (editing.value) {
      const res = await api.put(`/conceptos-gastos/${editing.value.id}`, form.value)
      const idx = items.value.findIndex(i => i.id === editing.value.id)
      if (idx !== -1) items.value[idx] = res.data
      showToast("Concepto actualizado", "success")
    } else {
      const res = await api.post("/conceptos-gastos/", form.value)
...
async function remove(item) {
  ...
  try {
    await api.delete(`/conceptos-gastos/${item.id}`)
