<template>
  <div class="page-container">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title"><i class="bi bi-cart3 me-2"></i>Conceptos de Compra</h1>
        <p class="page-subtitle">Catálogo de conceptos para registro de compras en tareas</p>
      </div>
    ...
    <div class="search-bar">
      ...
      <input v-model="search" placeholder="Buscar concepto..." class="search-input" />
    </div>

    <div v-if="filtered.length === 0" class="empty-state">
      <i class="bi bi-cart-x"></i>
      <p>{{ search ? 'Sin resultados para "' + search + '"' : 'No hay conceptos de compra registrados' }}</p>
    ...
    <div v-if="modal" class="modal-overlay" @click.self="modal = false">
      <div class="modal-card">
        <div class="modal-header">
          <h3>{{ editing ? 'Editar concepto' : 'Nuevo concepto de compra' }}</h3>
          <button class="modal-close" @click="modal = false"><i class="bi bi-x-lg"></i></button>
        </div>
        <div class="modal-body">
          <div class="fg">
            <label>Nombre *</label>
            <input v-model="form.name" class="form-control" placeholder="Ej: Materiales de obra, Equipos, Repuestos..." autofocus />
          </div>
          <div class="fg mt-3">
...
async function load() {
  try {
    const res = await api.get("/conceptos-compras/")
...
async function save() {
  ...
  try {
    if (editing.value) {
      const res = await api.put(`/conceptos-compras/${editing.value.id}`, form.value)
      const idx = items.value.findIndex(i => i.id === editing.value.id)
      if (idx !== -1) items.value[idx] = res.data
      showToast("Concepto actualizado", "success")
    } else {
      const res = await api.post("/conceptos-compras/", form.value)
...
async function remove(item) {
  ...
  try {
    await api.delete(`/conceptos-compras/${item.id}`)
