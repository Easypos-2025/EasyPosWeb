...
from sqlalchemy import Integer, String, Text, Float, Boolean, Date, SmallInteger, ForeignKey, TIMESTAMP, func
...
class Advertisement(Base):
    ...
    target_profile_id: Mapped[int]    = mapped_column(ForeignKey("business_profiles.id"), nullable=True)
    # pending | approved | active | paused | expired | rejected
    status:            Mapped[str]    = mapped_column(String(20), nullable=False, default="pending")
    slot_position:     Mapped[int]    = mapped_column(SmallInteger, nullable=True)   # 1, 2, 3
...
class AdPiece(Base):
    ...
    id:               Mapped[int]  = mapped_column(Integer, primary_key=True)
    advertisement_id: Mapped[int]  = mapped_column(ForeignKey("advertisements.id", ondelete="CASCADE"), nullable=False, index=True)
