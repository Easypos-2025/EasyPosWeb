...
def _ser(p: Product, db: Session) -> dict:
    cat = db.get(ProductCategory, p.category_id) if p.category_id else None
    ref = db.get(ProductReference, p.reference_id) if p.reference_id else None
    return {
        "id":                 p.id,
        ...
        "reference_name":     ref.name if ref else None,
        "inventory_behavior": p.inventory_behavior,
        "base_price":         float(p.base_price),
        "cost_price":         float(p.cost_price),
        "tax_rate":           float(p.tax_rate),
        "min_stock":          float(p.min_stock),
        "ask_price":          p.ask_price,
        "ask_description":    p.ask_description,
...
def list_products(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [_ser(p, db) for p in products]


@router.post("/")
def create_product(data: dict = Body(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    p = Product(
        ...
        ask_price=int(data.get("ask_price", 0)),
...
@router.put("/{pid}")
def update_product(pid: int, data: dict = Body(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
...
def get_recipe(pid: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    rows = db.query(ProductRecipe).filter(ProductRecipe.product_id == pid).all()
    result = []
    for r in rows:
        si = db.get(SupplyItem, r.supply_item_id)
        result.append({
            "id": r.id, "supply_item_id": r.supply_item_id,
...
def add_recipe_line(pid: int, data: dict = Body(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    r = ProductRecipe(
        ...
        qty_required=float(data.get("qty_required", 1)),
        unit_id=data.get("unit_id"),
    )
    db.add(r); db.commit(); db.refresh(r)
    si = db.get(SupplyItem, r.supply_item_id)
    return {"id": r.id, "supply_item_id": r.supply_item_id,
            "supply_item_name": si.name if si else "—",
            "qty_required": float(r.qty_required), "unit_id": r.unit_id}


@router.delete("/{pid}/recipe/{rid}")
def delete_recipe_line(pid: int, rid: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    r = db.query(ProductRecipe).filter(ProductRecipe.id == rid, ProductRecipe.product_id == pid).first()
    if not r:
...
def get_presentations(pid: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    rows = db.query(ProductPresentation).filter(ProductPresentation.product_id == pid, ProductPresentation.is_active == 1).all()
    return [{"id": r.id, "name": r.name, "factor": float(r.factor),
             "barcode": r.barcode, "price": float(r.price) if r.price is not None else None,
             "supply_item_id": r.supply_item_id, "is_active": r.is_active} for r in rows]


@router.post("/{pid}/presentations")
def add_presentation(pid: int, data: dict = Body(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    pr = ProductPresentation(
        product_id=pid,
        supply_item_id=data.get("supply_item_id"),
        name=data["name"].strip(),
    ...
    return {"id": pr.id, "name": pr.name, "factor": float(pr.factor),
            "barcode": pr.barcode, "price": float(pr.price) if pr.price is not None else None,
            "supply_item_id": pr.supply_item_id, "is_active": pr.is_active}


@router.delete("/{pid}/presentations/{prid}")
def delete_presentation(pid: int, prid: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = db.query(Product).filter(Product.id == pid, Product.company_id == current_user.company_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Producto no encontrado")
    pr = db.query(ProductPresentation).filter(ProductPresentation.id == prid, ProductPresentation.product_id == pid).first()
    if not pr:
