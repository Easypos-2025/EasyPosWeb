...
async def list_conceptos(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        ...
        .where(ConceptoGasto.company_id == current_user.company_id)
        .order_by(ConceptoGasto.name)
    )
...
async def create_concepto(
    ...
    result = await db.execute(
        select(ConceptoGasto).where(ConceptoGasto.company_id == current_user.company_id, ConceptoGasto.name == name)
    )
...
async def update_concepto(
    ...
    result = await db.execute(
        select(ConceptoGasto).where(ConceptoGasto.id == concepto_id, ConceptoGasto.company_id == current_user.company_id)
    )
